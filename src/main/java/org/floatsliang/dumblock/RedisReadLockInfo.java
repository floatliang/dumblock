 ...
 * @date 2020/5/14 2:27 PM
 */
public class RedisReadLockInfo extends RedisLockInfo {


    public RedisReadLockInfo(String lockKey, String lockVal, long timeout, long expireTimeout) {
        super(lockKey, lockVal, timeout, expireTimeout);
    }

    @Override
    public String getLockVal() {
        return super.getLockVal() + ":read";
    }
    ...
    public boolean unLockInRedis(Jedis jedis) {
        String luaScript = "local mode = redis.call('HGET', KEYS[1], 'mode');\n" +
                ...
                "\treturn 0;\n" +
                "end;\n" +
                "if (mode == 'read' and redis.call('HEXISTS', KEYS[1], ARGV[1]) == 1) then\n" +
    ...
    public boolean rechargeInRedis(Jedis jedis, long expireTimeout) {
        ...
        String luaScript = "if (redis.call('HGET', KEYS[1], 'mode') == 'read' and redis.call('HEXISTS', KEYS[1], ARGV[1]) == 1) then\n" +
